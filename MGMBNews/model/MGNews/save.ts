...
export default class model{
    constructor($tObj:any,$eObj:any) 
    {
     ...
     save=(obj:pub.NewsCtr)=>
     {
         if(self.titleSaveNuAry.length==0 && self.docSaveNuAry.length==0)
         {
             ...
             if(obj.update)
             {//標題儲存
                 self.titleSaveNuAry.push({ id: nu,name:obj.titleAry[main.pub.langNu],ck:false});
                 nu++;
             }
 
             if(obj.LABELupdate)
             {/** 移除深入閱讀標籤 */
                 self.titleSaveNuAry.push({ id: nu,name:"remove Label:",ck:false});
             }
             ...
             {/** 深入閱讀標籤 */
                 if(val1.update)
                 {
                     self.titleSaveNuAry.push({ id: nu,name:"read Label:"+val1.path.substring(0,9)+".../"+val1.nameAry[main.pub.langNu].substring(0,9)+"...",ck:false});
                 }
                 nu++;
             });
             ...
             {/** 移除其它標籤 */
                 self.titleSaveNuAry.push({ id: nu,name:"remove event Label:",ck:false});
             }
             ...
             {/** 其它標籤 */
                 if(val1.update)
                 {
                     self.titleSaveNuAry.push({ id: nu,name:"event Label:"+val1.path.substring(0,9)+".../"+val1.nameAry[main.pub.langNu].substring(0,9)+"...",ck:false});
                 }
                 nu++;
             });
             ...
             let docAryObj:Function = ()=>{
                 ...
                 {/** 段落標籤 */
                     ...
                     let imgSaveobj:Function=()=>
                     {
                            ...
                            {/** 文章圖片上傳 */
                                ...
                                val1.imgfileAry.forEach((val2,nu2)=>
                                {
                                    ...
                                    {
                                        waitImg++;
                                        let imgUpload:uploadView = { id: nu,name:"img/photo upload:"+val2.filename,ck:false};
                     ...
                     let saveContentDoc:Function=()=>
                     {
                         if(val1.update)
                         {
                            let doc:uploadView={ id: nu,name:"document:"+val1.content[main.pub.lang].title.substring(0,9)+".../"+val1.content[main.pub.lang].content.substring(0,9)+"...",ck:false};
                    ...
                    {//儲存 img description
                        if(val2.update)
                        {
                            ...
                            let imgTitle:uploadView={ id: nu,name:"img/photo title:"+val2.titleAry[main.pub.langNu].substring(0,9)+"...",ck:false};
    ...
    {
        Login((x)=>x.post("/ns/mg/mb/newsvideo").input({key:obj.key,path:docObj.path,
            ybe: docObj.ybe
        }),(e:any)=>
        {
     ...
     */
     private editContentDoc = (obj:pub.NewsCtr,docObj:pub.DocPathCtr,saveObj:uploadView,imgSaveobj:Function)=>
     {   
         /** 儲存段落計數(語系統計) */
        ...
        {//取得目前已load 語系(被修改一次性儲存)
            ...
            let runSaveLang:Function = ()=>
            {
                if(saveContent==0)
                {
                    ...
                    Login((x)=>x.post("/ns/mg/mb/newsdocedit").input({key:obj.key,path:docObj.path,
                        nu:getLangNu,
      ...
      * @param keyid 圖片上傳id
     */
     private imgDocUpload = (obj:pub.NewsCtr,docObj:pub.DocPathCtr,imgObj:pub.imgFileObj,saveObj:uploadView,keyid:string)=>
     {
         /** 分割上傳大小 */
        ...
        {//排隊佈屬
            ...
            let waitUpload:Function = ()=>
            {
                if(nowSplit==nu1)
                {
                    Login((x)=>x.post("/ns/mg/mb/newsimgupload").input({key:obj.key,path:docObj.path,imgdata:val1,tp:((catchImg.length-1!=nu1)?'':"complete")+nu1,keyid:keyid}),(e:any)=>
                    {
                        nowSplit++;
                        if(Number(e.error) == jEnum.Enum_SystemErrorCode.Null)
                        {
                            ...
                            if(imgObj.upsize==imgObj.uptotalsize)
                            {
                                ...
                                if(docObj.objImg.Jobj[("/ns/newsmgimg/"+obj.uid+"/"+obj.key) as any]==null || docObj.objImg.Jobj[("/ns/newsmgimg/"+obj.uid+"/"+obj.key) as any]==undefined)
                                {
                                    docObj.objImg.Jobj[("/ns/newsmgimg/"+obj.uid+"/"+obj.key) as any] = {} as any;
                                }
                                let getImgData:any = docObj.objImg.Jobj[("/ns/newsmgimg/"+obj.uid+"/"+obj.key) as any];
                                getImgData[("/ns/newsmgimg/"+obj.uid+"/"+obj.key+"/"+newImg.path) as any]=imgObj.base64;//注入圖片
                                docObj.objImg.key = "/ns/newsmgimg/"+obj.uid+"/"+obj.key;//注入儲存容器key
                                //--------------------完成上傳清除
                                ...
                                setTimeout(()=>
                                {
                                    ...
                                    {//圖片已上傳完畢
                                        ...
                                        obj.docAry.forEach((val,nu)=>{
                                            ...
                                            {//清空上傳資料input
                                                pb.el.id('newofile_'+obj.key+'_'+nu).get.value ="";
                                            }
                                        });
                                    }
                                },100);
     ...
     */
     private editContentImgDoc = (obj:pub.NewsCtr,docObj:pub.DocPathCtr,imgObj:pub.DocImgFileFormatCtr,saveObj:uploadView,oderSave:Function)=>
     {
         imgObj.update = false;
         Login((x)=>x.post("/ns/mg/mb/newsimgedit").input({key:obj.key,path:docObj.path,imgpath:imgObj.path,titleAry:JSON.stringify(imgObj.titleAry)}),(e:any)=>{
             if(Number(e.error) == jEnum.Enum_SystemErrorCode.Null)
             {
                 saveObj.ck = true;
             }
     ...
     */
     private editDoc = (val:pub.NewsCtr,saveLine:Array<uploadView>,docStepSave:Function)=>
     {
         Login((x)=>x.post("/ns/mg/mb/newsedit").input({key:val.key,title:JSON.stringify(val.titleAry), mdoc:JSON.stringify(val.mdocAry), readPath:JSON.stringify(val.readPathAry)
         ,nckey:val.nckey
         } as jDB.News ),(e:any)=>{
             if(Number(e.error) == jEnum.Enum_SystemErrorCode.Null)
             {
     ...
     */
     startReSave = ()=>
     {
         try
         {
             if(localStorage.getItem("psylnewssaveexist")!=null && localStorage.getItem("psylnewssave") !=null)
             {
                 pb.v(mt,"head_temp").async((eh:pub.mainHeadTemp)=>
                 {
                     if(pb.unixReNow()-Number(localStorage.getItem("psylnewssaveexist"))>540)
                     {//存活 9分鐘內資訊
                         mt.viewConfirm($t.getLang('mes').resavestart,()=>
                         {
                             let obj:pub.NewsCtr = JSON.parse(pb.Base64ToUTF8(localStorage.getItem("psylnewssave") as string));
                             ...
                             {//異常帳戶 清除資訊
                                 localStorage.removeItem("psylnewssaveexist");
                                 localStorage.removeItem("psylnewssave");
      ...
      * @param point error
      */
     private reSave(obj:pub.NewsCtr)
     {
        /** 確認是否已處理完成 */
        ...
        {//緩進3 秒
            ...
            try
            {
                localStorage.setItem("psylnewssaveexist",String(pb.unixReNow()));//已存在時間
                if(!ckComplete)
                {
                    let objGet:pub.NewsCtr = JSON.parse(JSON.stringify(obj));
                    (objGet.docAry as Array<pub.DocPathCtr>).forEach((val,nu)=>
                    {//清除 圖片暫存(localStorage max size 5MB)
                        val.imgfile = null;
                        val.imgfileAry = [];
                    });
                    localStorage.setItem("psylnewssave", pb.UTF8ToBase64(JSON.stringify(objGet)));
                }
                else
                {//完成清除 續存
                    localStorage.removeItem("psylnewssaveexist");
      ...
      * @param obj 
      */
     usUpdate=(obj:pub.NewsCtr):boolean=>
     {
         /** update已更動 */
