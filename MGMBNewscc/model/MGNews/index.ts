...
export default class model
{
    ...
    */
     ...
     */
     ...
     initLoad = ()=>
     {
      ...
      moreData=(val:pub.NewsccCtr)=>
      {
        ...
        Login(x2 => x2.post("/nscc/mg/mb/newsfile/"+val.key+"_"+ main.pub.langNu+"_"+val.uid),(obj2:any)=>
        {//載入段落 file
                ...
                val.docAry.forEach((val2:jDB.DocPath,nu2:number)=>
                {
                    ...
                    if(!imgStartLoad)
                    {
                        ...
                        setTimeout(()=>{
                            ...
                            {   
                                e3.forEach((val3,nu3)=>{
                                    $t.$an.main.loadImg('loadPhotocc'+val.key+'_'+val2.path+'_'+val3.split('.')[0],1000);//圖片載入完成動畫
                                });
                                /** 匹次載圖 */
    ...
    getFileFirst=(val:pub.NewsccCtr,fun:(obj:pub.NewsccCtr)=>void)=>
    {
        ...
        Login(x2 => x2.post("/nscc/mg/mb/newsfilefirst/"+val.key+"_"+ main.pub.langNu+"_"+val.uid),(obj2:any)=>
        {//載入段落 file
            if(Number(obj2.error) == jEnum.Enum_SystemErrorCode.Null)
            {
        ...
        pb.v(mt,"head_temp").async((e:pub.mainHeadTemp)=>
        {
            ...
            {//防連點
                pb.v($t,"toolvue").async(te=>{
                    Login(x => x.post("/nscc/mg/newstrace")
                    .input({
        ...
        pb.v(mt,"head_temp").async((e:pub.mainHeadTemp)=>
        {
            ...
            {//防連點
                ...
                pb.v($t,"toolvue").async(te=>{
                    ...
                    Login(x => x.post("/nscc/mg/newslist")
                    .input({
                        ser:te.InputSer,
                        pagetime:pagetime,
