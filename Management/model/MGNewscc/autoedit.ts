                                    ...
                                    .input({"uidAry":JSON.stringify(uidList)}),(e4:any)=>
                                    {
                                        if(Number(e4.error) == jEnum.Enum_SystemErrorCode.Null)
                                        {
                                            ...
                                            mblist.forEach((val:mbCtr,nu:number)=>{
                                                ...
                                                autockAry.forEach((val2:jDB.AutoLCK,nu2:number)=>
                                                {
                                                    if(val2.uid==val.uid)
                                                    {
                                                ...
                                                if(!dataExist)
                                                {//不在設定資料
                                                    val.auto = {mguid:"",ck:false,uid:""} as jDB.AutoLCK;
                                                }
                                            });
                                            e.mbList = mblist;
    ...
    */
    setAuto=(val:mbCtr)=>
    {
        pb.v(mt,"head_temp").async((he:pub.mainHeadTemp)=>{
            if(he.load==0)
            {
                Login((x)=>x.post("nscc/mg/ad/autoset")
