...
export default class model{
    ...
    /**
      ...
      * @param keyid 圖片上傳id
     */
     private imgDocUpload = (obj:pE.pCtr,imgObj:pub.imgFileObj,keyid:string)=>
     {
         /** 分割上傳大小 */
         ...
         while(uploadDataSize>0)
         {
            ...
            {//排隊佈屬
                ...
                let waitUpload:Function = ()=>
                {
                    if(nowSplit==nu1)
                    {
                        Login((x)=>x.post("/pc/mg/sys/imgupload").input({key:obj.key,imgdata:val1,tp:((catchImg.length-1!=nu1)?'':"complete")+nu1,keyid:keyid}),(e:any)=>
                        {
                            nowSplit++;
    ...
    */
    ...
    /** 刪除預覽上傳單檔 */
    imguploadDel = (val:pE.pCtr,del:number)=>
    {
    ...
    /** 圖檔上傳 設置-同步語系上傳暫存檔 */
    imgupload = (dataObj:pE.pCtr,files:any)=>
    {//同步
    ...
    save = (val:pE.pCtr)=>
    {
      ...
      * @param imgary 刪除圖片
      */
     remove = (obj:pE.pCtr,imgary:Array<String>)=>
     {
        mt.viewConfirm("是否確認刪除？",()=>{
            pb.v($t,"editview").async((e:any)=>{
                Login((x)=>x.post("/pc/mg/sys/delimg")
                .input({key:obj.key,imgary:JSON.stringify(imgary)}),(e2:any)=>{
                    if(Number(e2.error) == jEnum.Enum_SystemErrorCode.Null)
                    {
                        ...
                        mt.ViewAlertAtClose("刪除成功！",null,1);
                    }
                    else
                    {
                        mt.viewAlert("伺服器忙線中");
      ...
      * @param imgpath 圖片名
      */
     preImg =(obj:pE.pCtr,imgpath:string)=>
     {
        pb.v(mt,"head_temp").async((e:pub.mainHeadTemp)=>
        {
            if(e.load==0)
            {
                Login((x)=>x.post("/pc/mg/sys/imgpre")
    ...
    * @param imgpath 圖片名
     */
    nextImg =(obj:pE.pCtr,imgpath:string)=>
    {
        pb.v(mt,"head_temp").async((e:pub.mainHeadTemp)=>
        {
            if(e.load==0)
            {
                Login((x)=>x.post("/pc/mg/sys/imgnext")
                .input({key:obj.key,imgpath:imgpath}),(e:any)=>{
                    if(Number(e.error) == jEnum.Enum_SystemErrorCode.Null)
                    {
                        obj.imgAry= e.data;
