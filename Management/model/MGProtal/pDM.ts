...
export default class model{
    constructor($tObj:any,$eObj:any) 
    {
     ...
     /** 選擇起用語系 */
     chooseLang=(val:dmCtr,lang:number)=>
     {
         pb.v(mt,"head_temp").async((eh:pub.mainHeadTemp)=>
         {
             ...
             {//防連點
                 ...
                 Login((x)=>x.post("/ma/mg/dm/lang").input({key:val.key,lang:JSON.stringify(ary)} as jDB.News),(e:any)=>
                 {//語系注入
    ...
    */
     ...
     * @param imgpath 圖片名
    */
     preContentImgDoc = (obj:dmCtr,imgpath:string)=>{
        pb.v(mt,"head_temp").async((eh:pub.mainHeadTemp)=>
        {
            if(eh.load==0)
            {//防連點
                Login((x)=>x.post("/ma/mg/dm/imgpre").input({key:obj.key,imgpath:imgpath}),(e:any)=>{
                    if(Number(e.error) == jEnum.Enum_SystemErrorCode.Null)
                    {
                        obj.imgAry = e.data;
                    }
     ...
     * @param imgpath 圖片名
    */
    nextContentImgDoc = (obj:dmCtr,imgpath:string)=>{
        pb.v(mt,"head_temp").async((eh:pub.mainHeadTemp)=>
        {
            if(eh.load==0)
            {//防連點
                Login((x)=>x.post("/ma/mg/dm/imgnext").input({key:obj.key,imgpath:imgpath}),(e:any)=>{
                    if(Number(e.error) == jEnum.Enum_SystemErrorCode.Null)
                    {
                        obj.imgAry = e.data;
                    }
    ...
    del=(val:dmCtr)=>
    {
        pb.v(mt,"head_temp").async((he:pub.mainHeadTemp)=>{
            if(he.load==0)
            {
                ...
                Login((x)=>x.post("/ma/mg/dm/del")
                .input({key:val.key}),(e:any)=>
                {
                    if(Number(e.error) == jEnum.Enum_SystemErrorCode.Null)
                    {
                        /** 重建序列container */
                        let newDatalist:Array<dmCtr> = [];
                        $t.pDM.list.forEach((val2:dmCtr,nu:number)=>{//建立排序
                            if(val.key!=val2.key)
                            {
    ...
    showt=(val:dmCtr)=>
    {
        pb.v(mt,"head_temp").async((he:pub.mainHeadTemp)=>{
            if(he.load==0)
            {
                ...
                Login((x)=>x.post("/ma/mg/dm/tshow")
                .input({key:val.key}),(e:any)=>
                {
                    if(Number(e.error) == jEnum.Enum_SystemErrorCode.Null)
                    {
                        val.showt = (e.data as jDB.ProductDM).showt;
                        val.display = false;
                    }
                    else
                    {
    ...
    display=(val:dmCtr)=>
    {
        pb.v(mt,"head_temp").async((he:pub.mainHeadTemp)=>{
            if(he.load==0)
            {
                ...
                Login((x)=>x.post("/ma/mg/dm/display")
                .input({key:val.key}),(e:any)=>
                {
                    if(Number(e.error) == jEnum.Enum_SystemErrorCode.Null)
                    {
                        val.display = (e.data as jDB.ProductDM).display;
                    }
                    else
                    {
                        mt.viewAlert("伺服器忙線中");
    ...
    imgwp=(val:dmCtr)=>
    {
        pb.v(mt,"head_temp").async((he:pub.mainHeadTemp)=>{
            if(he.load==0)
            {
                ...
                Login((x)=>x.post("/ma/mg/dm/wallpaper")
                .input({key:val.key}),(e:any)=>
                {
                    if(Number(e.error) == jEnum.Enum_SystemErrorCode.Null)
                    {
                        val.imgwp = (e.data as jDB.ProductDM).imgwp;
                        val.display = false;
                    }
                    else
                    {
    ...
    insert=()=>
    {
        pb.v(mt,"head_temp").async((he:pub.mainHeadTemp)=>
        {
            if(he.load==0)
            {
                Login((x)=>x.post("/ma/mg/dm/edit")
                .input({key:""}),(e:any)=>
                {
                    if(Number(e.error) == jEnum.Enum_SystemErrorCode.Null)
                    {
                        /** 注入新資料 */
                        let addData:dmCtr = e.data;
                        ...
                        /** 重建序列container */
                        let newDatalist:Array<dmCtr> = [addData];
                        $t.pDM.list.forEach((val:dmCtr,nu:number)=>
                        {//建立排序
                            val.order += 1;
    ...
    /** 上移文章 */
    preDocument=(val:dmCtr)=>
    {
        pb.v(mt,"head_temp").async((he:pub.mainHeadTemp)=>{
            if(he.load==0)
            {
                Login((x)=>x.post("/ma/mg/dm/pre")
                .input({key:val.key}),(e:any)=>
                {
                    if(Number(e.error) == jEnum.Enum_SystemErrorCode.Null)
                    {
                        if(val.order != (e.data as Array<jDB.ProductDM>)[0].order)
                        {/* 防止未更動情況*/
                            /** 重建序列container */
                            let newDatalist:Array<dmCtr> = [];
                            let runFirst:boolean = true;
                            $t.pDM.list.forEach((val2:dmCtr,nu:number)=>
                            {//建立排序
                                if(val2.key==(e.data as Array<jDB.ProductDM>)[1].key)
                                {
                                    val2.order +=1;
                                    val.order -= 1;
                                    newDatalist.push(val);
                                ...
                                if(val2.key!=(e.data as Array<jDB.ProductDM>)[0].key)
                                {
                                    newDatalist.push(val2);
                                }
                            });
    ...
    /** 下移文章 */
    nextDocument=(val:dmCtr)=>
    {
        pb.v(mt,"head_temp").async((he:pub.mainHeadTemp)=>{
            if(he.load==0)
            {
                Login((x)=>x.post("/ma/mg/dm/next")
                .input({key:val.key}),(e:any)=>
                {
                    if(Number(e.error) == jEnum.Enum_SystemErrorCode.Null)
                    {
     ...
     */
    private edit=(val:dmCtr,fun:Function)=>
    {
        pb.v(mt,"head_temp").async((he:pub.mainHeadTemp)=>{
            if(he.load==0)
            {
    ...
    /**
      ...
      * @param keyid 圖片上傳id
     */
     private imgDocUpload = (obj:dmCtr,imgObj:pub.imgFileObj,keyid:string)=>
     {
         /** 分割上傳大小 */
         ...
         while(uploadDataSize>0)
         {
            ...
            {//排隊佈屬
                ...
                let waitUpload:Function = ()=>
                {
                    ...
                    {
                        $t.saveRun = true;
                        Login((x)=>x.post("/ma/mg/dm/imgupload").input({key:obj.key,imgdata:val1,tp:((catchImg.length-1!=nu1)?'':"complete")+nu1,keyid:keyid}),(e:any)=>
                        {
                            nowSplit++;
                            if(Number(e.error) == jEnum.Enum_SystemErrorCode.Null)
                            {
                                ...
                                if(imgObj.upsize==imgObj.uptotalsize)
                                {
                                    ...
                                    if(obj.objImg.Jobj[("/ma/mgpdimg/"+mt.head.mbdata.uid) as any]==null || obj.objImg.Jobj[("/ma/mgpdimg/"+mt.head.mbdata.uid) as any]==undefined)
                                    {
                                        obj.objImg.Jobj[("/ma/mgpdimg/"+mt.head.mbdata.uid) as any] = {} as any;
                                    }
                                    let getImgData:any = obj.objImg.Jobj[("/ma/mgpdimg/"+mt.head.mbdata.uid) as any];
                                    getImgData[("/ma/mgpdimg/"+mt.head.mbdata.uid+"/"+e.newImg) as any]=imgObj.base64;//注入圖片
                                    obj.objImg.key = "/ma/mgpdimg/"+mt.head.mbdata.uid;//注入儲存容器key
                                    obj.imgAry = [];
                                    obj.imgAry = e.data;
                                    obj.display = false;
                                    ...
                                    setTimeout(()=>
                                    {
                                        ...
                                        {//圖片已上傳完畢
                                            if(obj.imgfileAry.length==0)
                                            {
                                                obj.imgfile = null;
                                                obj.IMGupdate = false;
                                                pb.el.id('newofilePDM_'+obj.key).get.value ="";
                                            }
                                        $t.saveRun = false;
                                        }
                                    },100);
    ...
    */
    imguploadCancel = (val:dmCtr)=>
    {
        ...
        {//移除動畫
            ...
            val.imgfileAry = [];
            val.IMGupdate = false;
            pb.el.id('newofilePDM_'+val.key).get.value ="";
        }
        $t.$an.removeAllUpload('PDM'+val.key,run);
    }
     

    /** 刪除預覽上傳單檔 */
    imguploadDel = (val:dmCtr,del:number)=>
    {
        $t.$an.removeUpload('PDM'+del+val.key,()=>
        {//移除動畫
            val.IMGupdate = true;
    ...
    /** 圖檔上傳 設置-同步語系上傳暫存檔 */
    imgupload = (dataObj:dmCtr,files:any)=>
    {//同步
        ...
        $t.$an.dataRefrsh(dataObj.key+'PDMdocphoto');
    };
    /** 搜尋文章 data list
     * @param ser 搜尋關鍵字
     * @param init 是否初始化
    ...
    serData=(init:boolean)=>
    {
        pb.v(mt,"head_temp").async((he:pub.mainHeadTemp)=>{
            if(he.load==0)
            {
                ...
                Login((x)=>x.post("/ma/mg/dm/list")
                .input({ser:$t.pDM.ser,date:(($t.pDM.list.length>0)?$t.pDM.list[$t.pDM.list.length-1].date:0)}),(e:any)=>
                {
                    if(Number(e.error) == jEnum.Enum_SystemErrorCode.Null)
                    {
                        e.data.forEach((val2:dmCtr,nu2:number)=>
                        {//新增資料
                            let insert:boolean=true;
                            ($t.pDM.list as Array<jDB.ProductDM>).forEach((val1,nu1)=>
                            {//排除重覆
                            ...
                            if(insert)
                            {
                                ...
                                val2.objImg//緩儲圖片容器
                                .loadimgjson("/ma/mgpdimg/"+mt.head.mbdata.uid)//載入圖片
                                .input(val2.imgAry)
                                .async((e3,next3)=>
                                { 
                                    e3.forEach((val3,nu3)=>{
                                        $t.$an.loadImg("MGpDMPhoto"+(val3.split('.')[0])+val2.key,1000);//動畫
                                    });
                                    /** 匹次載圖 */
