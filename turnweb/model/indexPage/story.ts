...
/** 文章容器 */
interface stCtr extends jDB.Story
{
    /** 圖片儲存容器 */
    objImg:jObjM,
}
...
export default class model{
    ...
    {   
        ...
        {
            e3.tag.path = obj.key;
            e3.tag.tp = jEnum.Enum_docType.Story;
            e3.tag.show = false;
            e3.tag.content = null;
    ...
    serData=(init:boolean)=>
    {
        pb.v(mt,"head_temp").async((e:pub.mainHeadTemp)=>
        {
            if(e.load==0 || init)
            {
                Login(x=> x.post("/ma/main/story").input({date:(($t.story.list.length>0)?$t.story.list[$t.story.list.length-1].date:0)}),(obj:any)=>
                {
                    if(Number(obj.error) == jEnum.Enum_SystemErrorCode.Null)
                    {
                        pb.v($t,"storyVue").async(e2=>{
                            ...
                            (obj.data as Array<stCtr>).forEach((val,nu)=>
                            {//新增資料
                                let insert:boolean=true;
                                ($t.story.list as Array<stCtr>).forEach((val1,nu1)=>{
                                    if(val1.key==val.key)
                                    {
                                        insert=false;
                                    }
                                ...
                                if(insert)
                                {
                                    ...
                                    if(val.imgAry.length>0){
                                        ...
                                        val.imgAry = [];
                                        val.objImg//緩儲圖片容器
                                        .loadimgjson("/ma/stimg/"+val.key)//載入圖片
                                        .input(getImg)
